...
import type { AppData,ConfigFile, MiddlewareFunc, RouteData } from 'mdbCMS/types';
 ...
 * 3 - Add middle wares to api endPoints if they were set inside mdbCMS.js */
...
const handle:Handle = async({event,resolve})=>{
    ...
    // @ts-ignore set mdbCMS object to locals to avoid error
    event.locals.mdbCMS = { appData:{ assetProvider:undefined } }
    ...
    if(sessionValidated){
        ...
        event.locals.mdbCMS.userData = userData
        ...
        if(!appData){
            ...
            // set data to locals
            event.locals.mdbCMS.appData = newAppData.data
        }else event.locals.mdbCMS.appData = appData.data
        ...
        event.locals.mdbCMS.routes = routes
    }
