...
import utils from 'mdbCMS/lib/utils/utils.server';
import { json } from '@sveltejs/kit';
import { appCollectionNames } from "mdbCMS/lib/data"
import type { ApiRouteCreate,AppCollectionNames,AppRouteData } from 'mdbCMS/types';
...
export const POST: RequestHandler = async (event) => {
    ...
    if(newRouteRes.acknowledged){
        ...
        if(dev) utils.gen([...event.locals.mdbCMS.routes,jsonInputData])
        // return response
        const response:ApiRouteCreate['output'] = {
            error:false,
